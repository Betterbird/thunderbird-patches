...
1. `./betterbird.sh`
1. The Ansible build fails during the bootstrap stage, but it does pull the repositories and applies the patches. You can finish the build with `./mach build` and `./mach package`. This ultimately fails due to [bug 1737869](https://bugzilla.mozilla.org/show_bug.cgi?id=1737869).
